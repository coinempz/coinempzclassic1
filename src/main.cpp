...
uint256 hashGenesisBlock("0x00000616e1e31928af9fcee73a9dd87b9405d51ec0fe167868fe7e3fac5d813f");
...
const string strMessageMagic = "coinempzclassic Signed Message:\n";
...
{
    // coinempzclassic: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // coinempzclassic: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "coinempz classic genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1480873120;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1508396;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xa79aa5fd8e97b1b9b00fe62007e5fa72bb3b821afa0560f5c7ad72e2a6ccb7b4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdf, 0xc4, 0xbe, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("coinempzclassicMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("coinempzclassicMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static coinempzclassicMiner(CWallet *pwallet)
{
    printf("coinempzclassicMiner not supported!\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coinempzclassic-miner");
    ...
    {
        printf("coinempzclassicMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&coinempzclassicMiner, pwallet));
}
