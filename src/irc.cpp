...
// Copyright (c) 2011-2012 coinempzclassic Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("coinempzclassic-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #coinempzclassicTEST0\r");
            Send(hSocket, "WHO #coinempzclassicTEST0\r");
        } else {
            // randomly join #coinempzclassic00-#coinempzclassic99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #coinempzclassic%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #coinempzclassic%02d\r", channel_number).c_str());
        }
