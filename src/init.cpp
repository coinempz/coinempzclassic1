...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("coinempzclassic version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  coinempzclassicd [options]                     " + "\n" +
                  "  coinempzclassicd [options] <command> [params]  " + _("Send command to -server or coinempzclassicd") + "\n" +
                  "  coinempzclassicd [options] help                " + _("List commands") + "\n" +
                  "  coinempzclassicd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "coinempzclassic:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: coinempzclassic.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: coinempzclassicd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 33813 or testnet: 28760)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 33812 or testnet: 28761)") + "\n" +
        ...
        "\n" + _("SSL options: (see the coinempzclassic Wiki for SSL setup instructions)") + "\n" +
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. coinempzclassic is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("coinempzclassic version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "coinempzclassic server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of coinempzclassic") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart coinempzclassic to complete") << "\n";
