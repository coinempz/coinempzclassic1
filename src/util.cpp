...
#else
    const char* pszModule = "coinempzclassic";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "coinempzclassic";
#else
    fs::path pathRet;
    ...
    return pathRet / "coinempzclassic";
#else
    // Unix
    return pathRet / ".coinempzclassic";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "coinempzclassic.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "coinempzclassicd.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 15 * 60) // coinempzclassic: changed maximum adjust to 15 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong coinempzclassic will not work properly.");
